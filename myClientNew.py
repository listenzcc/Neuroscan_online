...
class ScanClient(object):
    ...
    def get_measurement_info(self):
        ...
        fResolution = self.basicInfo['fResolution'] = 0.00015# 0.00015  # 0.00014827
    ...
    def read_raw_buffer(self, nchan):
        ...
        if head[0][2] == 2:
            # dt = np.dtype(int)
            # dt = dt.newbyteorder('')
            buffer_array = np.frombuffer(buffer, '<i4')
            buffer_array_f = np.frombuffer(buffer, '<i')
        elif head[0][2] == 1:
            buffer_array = np.frombuffer(buffer, '<i2')
        else:
            buffer_array = None

        if buffer_array is not None:
            buffer_array = buffer_array.reshape(-1, nchan).T

        if buffer_array_f is not None:
            buffer_array_f = buffer_array_f.reshape(-1, nchan).T
            # print(np.max(buffer_array_f[-1]), buffer_array_f[-1])
        ...
        if buffer_array is not None:
            ...
            if buffer_array_f is not None:
                buffer_array[-1, :] = buffer_array_f[-1, :]
...
if __name__ == '__main__':
    ...
    # data=np.load('C:/Users/11273/data.npz')


    # c = ScanClient('10.0.180.151',4000)
    # c = ScanClient('127.0.0.1', 5555)
    c = ScanClient(server_host, server_port)
    ...
    time.sleep(1)
    # head,buff = _recv_head_raw()
    data=np.zeros((channel_num+1,0))
    for i in range(400):
        buff = c.read_raw_buffer(channel_num+1)
        # head,buff = c._recv_head_raw()
        # print(head)
        print('**************************')
        print(buff)
        print(buff.shape)
        data=np.concatenate([data,buff],axis=1)
        print('---------------------------')
        # print(buff)

    np.savez('C:/Users/11273/data.npz',data=data)
    print('---------------------------')
    print(test.dtype)
    print('---------------------------')
    print(test)
